...
DECISIONS_RECORDED = "capability_activation_followup_result_decisions_recorded"
DECISIONS_ALREADY_RECORDED = (
    "capability_activation_followup_result_decisions_already_recorded"
)
DECISIONS_NO_RESULTS = "capability_activation_followup_result_decisions_no_results"
REPORT_PATH = "docs/capability-activation-followup-decisions.md"


def decide_capability_activation_followup_results(
    root: Path,
    *,
    operator_id: str,
    selected_action: str,
    ...
    result_records = storage.list_capability_activation_followup_result_records()
    already_decided_result_ids = _already_decided_result_ids(storage, result_records)
    ready_records = [
    ...
    decision = storage.record_capability_activation_followup_result_decision(
        status=status,
    ...
    report_path.write_text(
        render_capability_activation_followup_result_decision_report(
            decision,
            ready_records,
        ),
        encoding="utf-8",
...
def render_capability_activation_followup_result_decision_line(
...
def render_capability_activation_followup_result_decision_report(
    decision: CapabilityActivationFollowupResultDecision,
    decided_records: list[CapabilityActivationFollowupResultRecord],
) -> str:
    lines = [
...
def _already_decided_result_ids(
    storage: Storage,
    result_records: list[CapabilityActivationFollowupResultRecord],
) -> set[str]:
...
def _render_result_line(record: CapabilityActivationFollowupResultRecord) -> str:
    return (
