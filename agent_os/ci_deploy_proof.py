...
CI_DEPLOY_PROOF_BLOCKED = "ci_deploy_proof_blocked"
OPERATOR_CI_DEPLOY_REVIEW_REQUIRED = "operator_ci_deploy_review_required"
NO_CI_DEPLOY_PROOF_CANDIDATE = "no_ci_deploy_proof_candidate"
KEEP_CI_DEPLOY_DISABLED = "keep_ci_deploy_disabled"
MANUAL_CI_DEPLOY_REVIEW_REQUIRED = "manual_ci_deploy_review_required"
KEEP_BROWSER_DESKTOP_ADAPTERS_DISABLED = "keep_browser_desktop_adapters_disabled"
CI_DEPLOY_BLOCKED = "blocked_until_browser_desktop_adapter_proof_and_operator_approval"
CI_DEPLOY_REVIEW_READY = "ready_for_operator_ci_deploy_review"
CI_DEPLOY_PROOF = "ci_deploy_proof"
CI_DEPLOY_PROOF_ITEM = "ci_deploy_evidence_contract"
NO_EFFECT = "none"
REPORT_PATH = "docs/ci-deploy-proof-checklist.md"


def write_ci_deploy_proof_checklist(
    root: Path,
) -> tuple[Path, CiDeployProofChecklist]:
    root = root.resolve()
    storage = Storage(root / ".agent" / "state.db")
    storage.initialize()

    adapter_checklists = storage.list_recent_browser_desktop_adapter_proof_checklists(
        limit=None,
    )
    (
        source_checklist,
    ...
    ) = _latest_real_cost_sourced_browser_desktop_adapter_proof(
        storage=storage,
        adapter_checklists=adapter_checklists,
    )
    if source_checklist is None:
        fallback_checklist = adapter_checklists[0] if adapter_checklists else None
        source_autonomous_scheduling_checklist = (
        ...
        if (
            ...
            and (
                source_autonomous_scheduling_checklist is None
                or source_remote_worker_checklist is None
    ...
    checklist = _checklist_from_latest_adapter_proof(
        storage=storage,
    ...
    report_path.write_text(
        render_ci_deploy_proof_checklist_report(
            checklist,
            source_checklist,
            source_autonomous_scheduling_checklist,
            source_remote_worker_checklist,
...
def render_ci_deploy_proof_checklist_report(
    checklist: CiDeployProofChecklist,
    source_checklist: BrowserDesktopAdapterProofChecklist | None,
    ...
    lines.extend(["", "## Source Browser Desktop Adapter Proof Checklist", ""])
    if source_checklist is None:
        lines.append("- none")
    else:
        lines.extend(
...
def _latest_real_cost_sourced_browser_desktop_adapter_proof(
    *,
    storage: Storage,
    adapter_checklists: list[BrowserDesktopAdapterProofChecklist],
) -> tuple[
    ...
    for adapter_checklist in adapter_checklists:
        autonomous_scheduling_checklist = (
            storage.get_autonomous_scheduling_proof_checklist(
                adapter_checklist.source_checklist_id
            )
        )
...
def render_ci_deploy_proof_checklist_line(
    checklist: CiDeployProofChecklist,
) -> str:
    return (
...
def render_checklist_item_line(item: dict[str, str]) -> str:
    parts = [
        ...
        f"recommended_ci_deploy_action={item['recommended_ci_deploy_action']}",
        f"ci_deploy_state={item['ci_deploy_state']}",
    ...
    parts.extend(
        [
            ...
            f"ci_deploy_effect={item['ci_deploy_effect']}",
...
def _checklist_from_latest_adapter_proof(
    *,
    storage: Storage,
    source_checklist: BrowserDesktopAdapterProofChecklist | None,
    report_path: str,
    ...
    if source_checklist is None:
        return storage.record_ci_deploy_proof_checklist(
            status=BROWSER_DESKTOP_ADAPTER_PROOF_CHECKLIST_MISSING,
            ...
            boundary_count=0,
            recommended_commands=["browser-desktop-adapter-proof-checklist"],
            reason="No browser/desktop adapter proof checklist exists yet.",
            checklist_items=[],
            report_path=report_path,
        )

    if (
        source_checklist.status != "operator_browser_desktop_adapter_review_required"
        ...
        return storage.record_ci_deploy_proof_checklist(
            ...
            reason=(
                "Latest browser/desktop adapter proof checklist is incomplete; "
    ...
    source_item = _adapter_source_item(source_checklist)
    checklist_items = (
        [_checklist_item_from_adapter_proof_item(source_item)]
    ...
    blocked_ci_deploy_proof_count = sum(
        ...
        if item["recommended_ci_deploy_action"] == KEEP_CI_DEPLOY_DISABLED
    )
    operator_review_required_count = sum(
        1
        for item in checklist_items
        if item["recommended_ci_deploy_action"]
        == MANUAL_CI_DEPLOY_REVIEW_REQUIRED
    )
    if blocked_ci_deploy_proof_count:
        status = CI_DEPLOY_PROOF_BLOCKED
        reason = (
            "CI Deploy proof remains blocked until browser/desktop adapter "
        ...
        status = OPERATOR_CI_DEPLOY_REVIEW_REQUIRED
        reason = (
            "CI Deploy proof is ready for manual operator review; no CI, "
            "deploy, adapter, scheduler, routing, or claim state changes were applied."
        )
    else:
        status = NO_CI_DEPLOY_PROOF_CANDIDATE
        reason = (
            "No CI Deploy proof checklist row was needed from the latest "
    ...
    return storage.record_ci_deploy_proof_checklist(
        ...
        blocked_adapter_proof_count=sum(
            1
            for item in checklist_items
            if item["source_adapter_action"] == KEEP_BROWSER_DESKTOP_ADAPTERS_DISABLED
        ),
...
def _adapter_source_item(
    source_checklist: BrowserDesktopAdapterProofChecklist,
) -> dict[str, str] | None:
    for item in source_checklist.checklist_items:
        if item["capability"] == "browser_desktop_adapters":
            return item
    return None


def _checklist_item_from_adapter_proof_item(
    item: dict[str, str],
) -> dict[str, str]:
    ci_deploy_blocked = (
        item["recommended_adapter_action"]
        == KEEP_BROWSER_DESKTOP_ADAPTERS_DISABLED
    )
    checklist_item = {
        "capability": CI_DEPLOY_PROOF,
        "proof_item": CI_DEPLOY_PROOF_ITEM,
        "recommended_ci_deploy_action": (
            KEEP_CI_DEPLOY_DISABLED
            if ci_deploy_blocked
            else MANUAL_CI_DEPLOY_REVIEW_REQUIRED
        ),
        "ci_deploy_state": (
            CI_DEPLOY_BLOCKED if ci_deploy_blocked else CI_DEPLOY_REVIEW_READY
        ),
        "source_adapter_action": item["recommended_adapter_action"],
        "source_adapter_state": item["adapter_state"],
