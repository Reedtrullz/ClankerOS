...
APPLICATION_NO_APPLICABLE_PROPOSALS = (
    "capability_activation_followup_result_effect_application_no_applicable_proposals"
)
REPORT_PATH = "docs/capability-activation-followup-result-effect-application.md"


def apply_capability_activation_followup_result_effects(
    root: Path,
    *,
    operator_id: str,
    selection_note: str,
    evidence_reference: str,
) -> tuple[Path, CapabilityActivationFollowupResultEffectApplication, list[Effect]]:
    root = root.resolve()
    ...
    application = (
        storage.record_capability_activation_followup_result_effect_application(
            status=status,
    ...
    report_path.write_text(
        render_capability_activation_followup_result_effect_application_report(
            application,
            proposed_effects,
            applied_effects,
            existing_applied_effects,
...
def render_capability_activation_followup_result_effect_application_line(
    application: CapabilityActivationFollowupResultEffectApplication,
) -> str:
    return (
...
def render_capability_activation_followup_result_effect_application_report(
    application: CapabilityActivationFollowupResultEffectApplication,
...
def _result_payload(
    ...
    return {
        ...
        "source_followup_task_id": payload.get("source_followup_task_id"),
        "source_contract_id": payload.get("source_contract_id"),
        "source_activation_decision_id": payload.get(
