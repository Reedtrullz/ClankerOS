...
CI_DEPLOY_PROOF_CHECKLIST_MISSING = "ci_deploy_proof_checklist_missing"
BUDGET_ENFORCEMENT_PROOF_BLOCKED = "budget_enforcement_proof_blocked"
OPERATOR_BUDGET_ENFORCEMENT_REVIEW_REQUIRED = (
    "operator_budget_enforcement_review_required"
)
NO_BUDGET_ENFORCEMENT_PROOF_CANDIDATE = "no_budget_enforcement_proof_candidate"
KEEP_BUDGET_ENFORCEMENT_DISABLED = "keep_budget_enforcement_disabled"
MANUAL_BUDGET_ENFORCEMENT_REVIEW_REQUIRED = (
    "manual_budget_enforcement_review_required"
)
KEEP_CI_DEPLOY_DISABLED = "keep_ci_deploy_disabled"
BUDGET_ENFORCEMENT_BLOCKED = (
    "blocked_until_ci_deploy_proof_and_operator_approval"
)
BUDGET_ENFORCEMENT_REVIEW_READY = (
    "ready_for_operator_budget_enforcement_review"
)
BUDGET_ENFORCEMENT = "budget_enforcement"
BUDGET_ENFORCEMENT_PROOF_ITEM = "budget_policy_simulation_contract"
NO_EFFECT = "none"
REPORT_PATH = "docs/budget-enforcement-proof-checklist.md"


def write_budget_enforcement_proof_checklist(
    root: Path,
) -> tuple[Path, BudgetEnforcementProofChecklist]:
    root = root.resolve()
    storage = Storage(root / ".agent" / "state.db")
    storage.initialize()

    ci_deploy_checklists = storage.list_recent_ci_deploy_proof_checklists(limit=None)
    (
        source_checklist,
    ...
    ) = _latest_real_cost_sourced_ci_deploy_proof(
        storage=storage,
        ci_deploy_checklists=ci_deploy_checklists,
    )
    if source_checklist is None:
        fallback_checklist = ci_deploy_checklists[0] if ci_deploy_checklists else None
        source_browser_desktop_adapter_checklist = (
    ...
    checklist = _checklist_from_latest_ci_deploy_proof(
        storage=storage,
    ...
    report_path.write_text(
        render_budget_enforcement_proof_checklist_report(
            checklist,
            source_checklist,
            source_browser_desktop_adapter_checklist,
            source_autonomous_scheduling_checklist,
...
def render_budget_enforcement_proof_checklist_report(
    checklist: BudgetEnforcementProofChecklist,
    source_checklist: CiDeployProofChecklist | None,
    ...
    lines = [
        ...
        "",
        "## Budget Enforcement Proof Items",
        "",
    ]
    ...
    lines.extend(["", "## Source CI Deploy Proof Checklist", ""])
    if source_checklist is None:
        lines.append("- none")
    else:
        lines.extend(
    ...
    lines.extend(
        [
            ...
            "## Non-Claims",
            "",
            "- Report-only local Budget Enforcement proof checklist.",
...
def _latest_real_cost_sourced_ci_deploy_proof(
    *,
    storage: Storage,
    ci_deploy_checklists: list[CiDeployProofChecklist],
) -> tuple[
    ...
    for ci_deploy_checklist in ci_deploy_checklists:
...
def render_budget_enforcement_proof_checklist_line(
    checklist: BudgetEnforcementProofChecklist,
) -> str:
    return (
...
def render_checklist_item_line(item: dict[str, str]) -> str:
    parts = [
        ...
        f"recommended_budget_action={item['recommended_budget_action']}",
        f"budget_enforcement_state={item['budget_enforcement_state']}",
    ...
    parts.extend(
        [
            ...
            f"budget_enforcement_effect={item['budget_enforcement_effect']}",
...
def _checklist_from_latest_ci_deploy_proof(
    *,
    storage: Storage,
    source_checklist: CiDeployProofChecklist | None,
    report_path: str,
    ...
    if source_checklist is None:
        return storage.record_budget_enforcement_proof_checklist(
            status=CI_DEPLOY_PROOF_CHECKLIST_MISSING,
            ...
            boundary_count=0,
            recommended_commands=["ci-deploy-proof-checklist"],
            reason="No CI Deploy proof checklist exists yet.",
            checklist_items=[],
            report_path=report_path,
        )

    if (
        source_checklist.status != "operator_ci_deploy_review_required"
        ...
        return storage.record_budget_enforcement_proof_checklist(
            ...
            reason=(
                "Latest CI Deploy proof checklist is incomplete; run the "
                "recommended upstream command before checking Budget "
                "Enforcement proof."
            ),
            checklist_items=[],
            report_path=report_path,
    ...
    source_item = _ci_deploy_source_item(source_checklist)
    checklist_items = (
        [_checklist_item_from_ci_deploy_proof_item(source_item)]
    ...
    blocked_budget_enforcement_proof_count = sum(
        ...
        if item["recommended_budget_action"] == KEEP_BUDGET_ENFORCEMENT_DISABLED
    )
    operator_review_required_count = sum(
        1
        for item in checklist_items
        if item["recommended_budget_action"]
        == MANUAL_BUDGET_ENFORCEMENT_REVIEW_REQUIRED
    )
    if blocked_budget_enforcement_proof_count:
        status = BUDGET_ENFORCEMENT_PROOF_BLOCKED
        reason = (
            "Budget Enforcement proof remains blocked until CI Deploy proof, "
            "browser/desktop adapter proof, autonomous scheduling proof, "
            "remote worker proof, hosted dashboard proof, cost tracking, "
            "retry review, required evidence, and operator approvals are present."
        )
    elif operator_review_required_count:
        status = OPERATOR_BUDGET_ENFORCEMENT_REVIEW_REQUIRED
        reason = (
            "Budget Enforcement proof is ready for manual operator review; "
            "no budget, CI, deploy, adapter, scheduler, routing, or claim "
            "state changes were applied."
        )
    else:
        status = NO_BUDGET_ENFORCEMENT_PROOF_CANDIDATE
        reason = (
            "No Budget Enforcement proof checklist row was needed from the "
            "latest CI Deploy proof checklist."
        )
...
def _ci_deploy_source_item(
    source_checklist: CiDeployProofChecklist,
) -> dict[str, str] | None:
    for item in source_checklist.checklist_items:
        if item["capability"] == "ci_deploy_proof":
            return item
    return None


def _checklist_item_from_ci_deploy_proof_item(
    item: dict[str, str],
) -> dict[str, str]:
    budget_enforcement_blocked = (
        item["recommended_ci_deploy_action"] == KEEP_CI_DEPLOY_DISABLED
    )
    checklist_item = {
        "capability": BUDGET_ENFORCEMENT,
        "proof_item": BUDGET_ENFORCEMENT_PROOF_ITEM,
        "recommended_budget_action": (
            KEEP_BUDGET_ENFORCEMENT_DISABLED
            if budget_enforcement_blocked
            else MANUAL_BUDGET_ENFORCEMENT_REVIEW_REQUIRED
        ),
        "budget_enforcement_state": (
            BUDGET_ENFORCEMENT_BLOCKED
            if budget_enforcement_blocked
            else BUDGET_ENFORCEMENT_REVIEW_READY
        ),
        "source_ci_deploy_action": item["recommended_ci_deploy_action"],
        "source_ci_deploy_state": item["ci_deploy_state"],
        ...
        "budget_enforcement_effect": NO_EFFECT,
    ...
    if all(key in item for key in optional_proof_keys):
        for key in optional_proof_keys:
