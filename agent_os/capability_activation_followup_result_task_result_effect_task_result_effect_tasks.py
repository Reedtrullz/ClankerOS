...
GOAL_DESCRIPTION = (
    "Prepare downstream proof work from applied downstream result effect task result decision effects."
)


def write_capability_activation_followup_result_task_result_effect_task_result_effect_tasks(
    root: Path,
) -> tuple[
    Path,
    CapabilityActivationFollowupResultTaskResultEffectTaskResultEffectTaskBatch,
    ...
    applied_effects = _applied_downstream_result_effect_task_result_effects(storage)
    ...
    batch = (
        storage.record_capability_activation_followup_result_task_result_effect_task_result_effect_task_batch(
            status=status,
    ...
    report_path.write_text(
        render_capability_activation_followup_result_task_result_effect_task_result_effect_tasks_report(
            batch,
            created_tasks,
            existing_tasks,
            applied_effects,
...
def render_capability_activation_followup_result_task_result_effect_task_result_effect_task_batch_line(
    batch: CapabilityActivationFollowupResultTaskResultEffectTaskResultEffectTaskBatch,
) -> str:
    return (
...
def render_capability_activation_followup_result_task_result_effect_task_result_effect_tasks_report(
    batch: CapabilityActivationFollowupResultTaskResultEffectTaskResultEffectTaskBatch,
    created_tasks: list[Task],
    ...
    lines.extend(["", "## Applied Downstream Result Effect Task Result Effects", ""])
    if applied_effects:
        lines.extend(
            render_capability_activation_followup_result_task_result_effect_task_result_effect_proposal_line(
                effect
...
def _applied_downstream_result_effect_task_result_effects(
    storage: Storage,
) -> list[Effect]:
    return [
        effect
...
def _task_description(effect: Effect) -> str:
    return (
        ...
        f"from applied downstream result effect task result decision effect "
        f"{effect.id}; keep activation blocked until fresh proof and explicit "
...
def _verification_plan(effect: Effect) -> dict[str, object]:
    ...
    return {
        ...
        "source_application_effect_id": result.get("source_effect_id", "none"),
        ...
        "non_claims": [
            ...
            "does_not_mutate_downstream_result_effect_task_result_records",
        ],
    }
