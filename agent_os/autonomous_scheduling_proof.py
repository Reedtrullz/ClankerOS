...
REMOTE_WORKER_PROOF_CHECKLIST_MISSING = "remote_worker_proof_checklist_missing"
AUTONOMOUS_SCHEDULING_PROOF_BLOCKED = "autonomous_scheduling_proof_blocked"
OPERATOR_AUTONOMOUS_SCHEDULING_REVIEW_REQUIRED = (
    "operator_autonomous_scheduling_review_required"
)
NO_AUTONOMOUS_SCHEDULING_PROOF_CANDIDATE = (
    "no_autonomous_scheduling_proof_candidate"
)
KEEP_AUTONOMOUS_SCHEDULING_DISABLED = "keep_autonomous_scheduling_disabled"
MANUAL_AUTONOMOUS_SCHEDULING_REVIEW_REQUIRED = (
    "manual_autonomous_scheduling_review_required"
)
KEEP_REMOTE_WORKERS_DISABLED = "keep_remote_workers_disabled"
SCHEDULING_BLOCKED = "blocked_until_remote_worker_proof_and_operator_approval"
SCHEDULING_REVIEW_READY = "ready_for_operator_autonomous_scheduling_review"
AUTONOMOUS_SCHEDULING = "autonomous_scheduling"
AUTONOMOUS_SCHEDULING_PROOF_ITEM = "scheduler_policy_dry_run"
NO_EFFECT = "none"
REPORT_PATH = "docs/autonomous-scheduling-proof-checklist.md"


def write_autonomous_scheduling_proof_checklist(
    root: Path,
) -> tuple[Path, AutonomousSchedulingProofChecklist]:
    root = root.resolve()
    storage = Storage(root / ".agent" / "state.db")
    storage.initialize()

    worker_checklists = storage.list_recent_remote_worker_proof_checklists(
        limit=None,
    )
    (
        source_checklist,
    ...
    ) = _latest_real_cost_sourced_remote_worker_proof(
        storage=storage,
        worker_checklists=worker_checklists,
    )
    if source_checklist is None:
        fallback_checklist = worker_checklists[0] if worker_checklists else None
        source_hosted_dashboard_checklist = (
        ...
        if (
            ...
            and (
                source_hosted_dashboard_checklist is None
                or source_hosted_dashboard_checklist.source_kind
                != "real_cost_tracking_proof_checklist"
            )
        ):
            source_checklist = fallback_checklist
    checklist = _checklist_from_latest_remote_worker_proof(
        storage=storage,
    ...
    report_path.write_text(
        render_autonomous_scheduling_proof_checklist_report(
            checklist,
            source_checklist,
            source_hosted_dashboard_checklist,
            source_real_cost_tracking_checklist,
...
def render_autonomous_scheduling_proof_checklist_report(
    checklist: AutonomousSchedulingProofChecklist,
    source_checklist: RemoteWorkerProofChecklist | None,
    ...
    lines = [
        ...
        "",
        "## Autonomous Scheduling Proof Items",
        "",
    ]
    ...
    lines.extend(["", "## Source Remote Worker Proof Checklist", ""])
    if source_checklist is None:
        lines.append("- none")
    else:
        lines.extend(
    ...
    lines.extend(
        [
            ...
            "## Non-Claims",
            "",
            "- Report-only local autonomous scheduling proof checklist.",
...
def _latest_real_cost_sourced_remote_worker_proof(
    *,
    storage: Storage,
    worker_checklists: list[RemoteWorkerProofChecklist],
) -> tuple[
    RemoteWorkerProofChecklist | None,
    HostedDashboardProofChecklist | None,
    RealCostTrackingProofChecklist | None,
]:
    for worker_checklist in worker_checklists:
        hosted_dashboard_checklist = storage.get_hosted_dashboard_proof_checklist(
            worker_checklist.source_checklist_id
        )
        if (
...
def render_autonomous_scheduling_proof_checklist_line(
    checklist: AutonomousSchedulingProofChecklist,
) -> str:
    return (
...
def render_checklist_item_line(item: dict[str, str]) -> str:
    parts = [
        ...
        f"recommended_scheduling_action={item['recommended_scheduling_action']}",
        f"scheduling_state={item['scheduling_state']}",
    ...
    parts.extend(
        [
            ...
            f"scheduling_effect={item['scheduling_effect']}",
...
def _checklist_from_latest_remote_worker_proof(
    *,
    storage: Storage,
    source_checklist: RemoteWorkerProofChecklist | None,
    report_path: str,
    ...
    if source_checklist is None:
        return storage.record_autonomous_scheduling_proof_checklist(
            status=REMOTE_WORKER_PROOF_CHECKLIST_MISSING,
            ...
            boundary_count=0,
            recommended_commands=["remote-worker-proof-checklist"],
            reason="No remote worker proof checklist exists yet.",
            checklist_items=[],
            report_path=report_path,
        )

    if (
        source_checklist.status != "operator_remote_worker_review_required"
        ...
        return storage.record_autonomous_scheduling_proof_checklist(
            ...
            reason=(
                "Latest remote worker proof checklist is incomplete; run the "
                "recommended upstream command before checking autonomous "
                "scheduling proof."
            ),
            checklist_items=[],
            report_path=report_path,
    ...
    source_item = _remote_worker_source_item(source_checklist)
    checklist_items = (
        [_checklist_item_from_remote_worker_proof_item(source_item)]
    ...
    blocked_scheduling_proof_count = sum(
        ...
        if item["recommended_scheduling_action"]
    ...
    if blocked_scheduling_proof_count:
        status = AUTONOMOUS_SCHEDULING_PROOF_BLOCKED
        reason = (
            "Autonomous scheduling proof remains blocked until remote worker "
            "proof, hosted dashboard proof, cost tracking, retry review, "
            "required evidence, and operator approvals are present."
        )
    elif operator_review_required_count:
        status = OPERATOR_AUTONOMOUS_SCHEDULING_REVIEW_REQUIRED
        reason = (
            "Autonomous scheduling proof is ready for manual operator review; "
            "no scheduler, routing, or claim state changes were applied."
        )
    else:
        status = NO_AUTONOMOUS_SCHEDULING_PROOF_CANDIDATE
        reason = (
            "No autonomous scheduling proof checklist row was needed from the "
            "latest remote worker proof checklist."
        )
...
def _remote_worker_source_item(
    source_checklist: RemoteWorkerProofChecklist,
) -> dict[str, str] | None:
    for item in source_checklist.checklist_items:
        if item["capability"] == "remote_workers":
            return item
    return None


def _checklist_item_from_remote_worker_proof_item(
    item: dict[str, str],
) -> dict[str, str]:
    scheduling_blocked = (
        item["recommended_worker_action"] == KEEP_REMOTE_WORKERS_DISABLED
    )
    checklist_item = {
        "capability": AUTONOMOUS_SCHEDULING,
        "proof_item": AUTONOMOUS_SCHEDULING_PROOF_ITEM,
        "recommended_scheduling_action": (
            KEEP_AUTONOMOUS_SCHEDULING_DISABLED
            if scheduling_blocked
            else MANUAL_AUTONOMOUS_SCHEDULING_REVIEW_REQUIRED
        ),
        "scheduling_state": (
            SCHEDULING_BLOCKED if scheduling_blocked else SCHEDULING_REVIEW_READY
        ),
        "source_worker_action": item["recommended_worker_action"],
        "source_worker_state": item["worker_state"],
