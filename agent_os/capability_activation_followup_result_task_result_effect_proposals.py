...
IDEMPOTENCY_PREFIX = "capability-followup-result-task-result-decision-effect:"
EFFECT_TYPE = "capability_followup_result_task_blocked_result_proposal"


@dataclass(frozen=True)
class CapabilityActivationFollowupResultTaskResultEffectProposalSummary:
    status: str
...
def write_capability_activation_followup_result_task_result_effect_proposals(
    root: Path,
) -> tuple[
    Path,
    CapabilityActivationFollowupResultTaskResultEffectProposalSummary,
    list[Effect],
]:
    root = root.resolve()
    ...
    existing_effects = _capability_followup_task_result_decision_effects(storage)
    ...
    for decision, result_record in accepted_pairs:
        ...
        created_effects.append(
            storage.record_effect(
                ...
                compensation_plan={
                    "required": False,
                    "reason": "proposal_only_downstream_blocked_activation_preserved",
                },
                result_json={
    ...
    summary = CapabilityActivationFollowupResultTaskResultEffectProposalSummary(
    ...
    report_path.write_text(
        render_capability_activation_followup_result_task_result_effect_proposals_report(
            summary,
            accepted_pairs,
            created_effects,
            existing_effects,
...
def render_capability_activation_followup_result_task_result_effect_proposal_line(
    effect: Effect,
) -> str:
...
def render_capability_activation_followup_result_task_result_effect_proposals_report(
    summary: CapabilityActivationFollowupResultTaskResultEffectProposalSummary,
    accepted_pairs: list[
        tuple[
            CapabilityActivationFollowupResultTaskResultDecision,
...
def _accepted_decision_result_pairs(
    ...
    result_records_by_id = {
        record.id: record
        for record in storage.list_capability_activation_followup_result_task_result_records()
    }
    pairs = []
    for decision in (
...
def _capability_followup_task_result_decision_effects(
    storage: Storage,
) -> list[Effect]:
...
def _idempotency_key(
    decision: CapabilityActivationFollowupResultTaskResultDecision,
    result_record: CapabilityActivationFollowupResultTaskResultRecord,
) -> str:
...
def _payload_for_result(
    ...
    return {
        ...
        "source_followup_result_id": result_record.source_result_id,
...
def _render_accepted_result_line(
    ...
    return (
        ...
        f"source_result={record.source_result_id} "
