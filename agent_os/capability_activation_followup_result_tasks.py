...
TASKS_RECORDED = "capability_activation_followup_result_tasks_recorded"
...
GOAL_DESCRIPTION = (
    "Prepare downstream proof work from applied capability follow-up result effects."
)


def write_capability_activation_followup_result_tasks(
    root: Path,
) -> tuple[Path, CapabilityActivationFollowupResultTaskBatch, list[Task], list[Task], list[Effect]]:
    root = root.resolve()
    storage = Storage(root / ".agent" / "state.db")
    storage.initialize()

    applied_effects = _applied_followup_result_effects(storage)
    ...
    batch = storage.record_capability_activation_followup_result_task_batch(
        status=status,
        source_application_id=_latest_application_id(storage, applied_effects),
        applied_followup_effect_count=len(applied_effects),
    ...
    report_path.write_text(
        render_capability_activation_followup_result_tasks_report(
            batch,
            created_tasks,
            existing_tasks,
            applied_effects,
...
def render_capability_activation_followup_result_task_batch_line(
    batch: CapabilityActivationFollowupResultTaskBatch,
) -> str:
    return (
...
def render_capability_activation_followup_result_tasks_report(
    batch: CapabilityActivationFollowupResultTaskBatch,
    created_tasks: list[Task],
    ...
    lines.extend(["", "## Applied Follow-Up Result Effects", ""])
    if applied_effects:
        lines.extend(
...
def _applied_followup_result_effects(storage: Storage) -> list[Effect]:
    return [
        effect
...
def _task_description(effect: Effect) -> str:
    return (
        f"Prepare the next evidence plan for capability {effect.capability} from "
        f"applied follow-up result effect {effect.id}; keep activation blocked "
        "until fresh proof and explicit operator approval exist."
    )
...
def _verification_plan(effect: Effect) -> dict[str, object]:
    ...
    return {
        "type": "capability_activation_followup_result_downstream_task",
        "capability": effect.capability,
        "source_effect_id": effect.id,
        "source_application_id": result.get("application_id", "none"),
        "source_result_id": result.get("source_result_id", "none"),
...
def _task_evidence(effect: Effect) -> dict[str, object]:
    ...
    return {
        ...
        "source_followup_task_id": result.get("source_followup_task_id", "none"),
        "source_contract_id": result.get("source_contract_id", "none"),
        "source_activation_decision_id": result.get(
            "source_activation_decision_id",
            "none",
        ),
        "capability": effect.capability,
        "selected_action": result.get("selected_action", "none"),
        ...
        "task_origin": "applied_capability_followup_result_effect",
    }
...
def _render_task_line(task: Task) -> str:
    return (
        ...
        f"source_result={task.evidence.get('source_result_id')} "
