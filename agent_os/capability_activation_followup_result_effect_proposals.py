...
PROPOSALS_RECORDED = "capability_activation_followup_result_effect_proposals_recorded"
...
REPORT_PATH = "docs/capability-activation-followup-result-effect-proposals.md"
IDEMPOTENCY_PREFIX = "capability-followup-decision-effect:"
EFFECT_TYPE = "capability_followup_blocked_result_proposal"


@dataclass(frozen=True)
class CapabilityActivationFollowupResultEffectProposalSummary:
    status: str
...
def write_capability_activation_followup_result_effect_proposals(
    root: Path,
) -> tuple[
    Path,
    CapabilityActivationFollowupResultEffectProposalSummary,
    list[Effect],
]:
    root = root.resolve()
    ...
    existing_effects = _capability_followup_decision_effects(storage)
    ...
    for decision, result_record in accepted_pairs:
        ...
        created_effects.append(
            storage.record_effect(
                run_id=decision.id,
                task_id=result_record.followup_task_id,
                ...
                compensation_plan={
                    "required": False,
                    "reason": "proposal_only_blocked_activation_preserved",
                },
                result_json={
    ...
    summary = CapabilityActivationFollowupResultEffectProposalSummary(
    ...
    report_path.write_text(
        render_capability_activation_followup_result_effect_proposals_report(
            summary,
            accepted_pairs,
            created_effects,
            existing_effects,
...
def render_capability_activation_followup_result_effect_proposal_line(
    effect: Effect,
) -> str:
    source_result_id = effect.proposed_payload.get("source_result_id", "unknown")
    return (
...
def render_capability_activation_followup_result_effect_proposals_report(
    summary: CapabilityActivationFollowupResultEffectProposalSummary,
    accepted_pairs: list[
        tuple[
            CapabilityActivationFollowupResultDecision,
    ...
    lines.extend(["", "## Accepted Follow-Up Results", ""])
    if accepted_pairs:
        lines.extend(
...
def _accepted_decision_result_pairs(
    ...
    result_records_by_id = {
        record.id: record
        for record in storage.list_capability_activation_followup_result_records()
    }
    pairs = []
    for decision in storage.list_recent_capability_activation_followup_result_decisions(
        limit=None,
    ):
...
def _capability_followup_decision_effects(storage: Storage) -> list[Effect]:
...
def _idempotency_key(
    decision: CapabilityActivationFollowupResultDecision,
    result_record: CapabilityActivationFollowupResultRecord,
) -> str:
...
def _payload_for_result(
    ...
    return {
        ...
        "source_result_id": result_record.id,
        "source_delegation_id": result_record.delegation_id,
        "source_followup_task_id": result_record.followup_task_id,
        "source_contract_id": result_record.contract_id,
        "source_activation_decision_id": result_record.decision_id,
...
def _render_accepted_result_line(
    ...
    return (
        ...
        f"delegation={record.delegation_id} task={record.followup_task_id} "
