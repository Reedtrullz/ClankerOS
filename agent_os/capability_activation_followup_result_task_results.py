...
from agent_os.capability_activation_followup_result_tasks import TASK_TYPE
...
RESULTS_RECORDED = "capability_activation_followup_result_task_results_recorded"
...
REPORT_PATH = "docs/capability-activation-followup-result-task-results.md"


def write_capability_activation_followup_result_task_results(
    root: Path,
) -> tuple[
    Path,
    CapabilityActivationFollowupResultTaskResultBatch,
    ...
    created_records: list[CapabilityActivationFollowupResultTaskResultRecord] = []
    existing_records: list[CapabilityActivationFollowupResultTaskResultRecord] = []
    for delegation in completed_delegations:
        ...
        created_records.append(
            storage.record_capability_activation_followup_result_task_result(
                delegation_id=delegation.id,
                downstream_task_id=delegation.parent_task_id,
                source_result_id=str(task.evidence.get("source_result_id", "")),
    ...
    batch = storage.record_capability_activation_followup_result_task_result_batch(
        status=status,
    ...
    report_path.write_text(
        render_capability_activation_followup_result_task_results_report(
            batch,
            created_records,
            existing_records,
...
def render_capability_activation_followup_result_task_result_batch_line(
    batch: CapabilityActivationFollowupResultTaskResultBatch,
) -> str:
    return (
...
def render_capability_activation_followup_result_task_results_report(
    batch: CapabilityActivationFollowupResultTaskResultBatch,
    created_records: list[CapabilityActivationFollowupResultTaskResultRecord],
    existing_records: list[CapabilityActivationFollowupResultTaskResultRecord],
...
def _existing_record_for_delegation(
    ...
    return (
        storage.get_capability_activation_followup_result_task_result_by_idempotency_key(
            _idempotency_key(delegation),
        )
        or storage.get_capability_activation_followup_result_task_result_by_idempotency_key(
...
def _result_payload(
    ...
    return {
        "delegation_id": delegation.id,
        "downstream_task_id": delegation.parent_task_id,
        "source_result_id": task.evidence.get("source_result_id"),
        ...
        "source_followup_task_id": task.evidence.get("source_followup_task_id"),
...
def _idempotency_key(delegation: SubagentDelegation) -> str:
    return f"capability_activation_followup_result_task_result:{delegation.id}"
...
def _render_result_record_line(
    record: CapabilityActivationFollowupResultTaskResultRecord,
) -> str:
    return (
        f"- result={record.id} delegation={record.delegation_id} "
        f"task={record.downstream_task_id} source_result={record.source_result_id} "
