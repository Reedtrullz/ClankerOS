...
TASKS_RECORDED = (
    "capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_tasks_recorded"
)
...
TASKS_NO_APPLIED_EFFECTS = (
    "capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_tasks_no_applied_effects"
)
REPORT_PATH = (
...
GOAL_DESCRIPTION = (
    "Prepare downstream proof work from applied downstream result effect task result effect task result decision effects."
)


def write_capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_tasks(
    root: Path,
) -> tuple[
    Path,
    CapabilityActivationFollowupResultTaskResultEffectTaskResultEffectTaskResultEffectTaskBatch,
    ...
    applied_effects = _applied_downstream_result_effect_task_result_effect_task_result_effects(
        storage
    )
    ...
    batch = (
        storage.record_capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_task_batch(
            status=status,
    ...
    report_path.write_text(
        render_capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_tasks_report(
            batch,
            created_tasks,
            existing_tasks,
            applied_effects,
...
def render_capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_task_batch_line(
    batch: CapabilityActivationFollowupResultTaskResultEffectTaskResultEffectTaskResultEffectTaskBatch,
) -> str:
    return (
...
def render_capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_tasks_report(
    batch: CapabilityActivationFollowupResultTaskResultEffectTaskResultEffectTaskResultEffectTaskBatch,
    created_tasks: list[Task],
    ...
    lines.extend(
        ["", "## Applied Downstream Result Effect Task Result Effect Task Result Effects", ""]
    )
    if applied_effects:
        lines.extend(
            render_capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_proposal_line(
    ...
    lines.extend(
        [
            ...
            "- Does not mutate downstream result effect task result effect task result records.",
...
def _applied_downstream_result_effect_task_result_effect_task_result_effects(
    storage: Storage,
) -> list[Effect]:
    return [
        effect
...
def _task_description(effect: Effect) -> str:
    return (
        ...
        f"decision effect {effect.id}; keep activation blocked until fresh "
        "proof and explicit operator approval exist."
    )
...
def _verification_plan(effect: Effect) -> dict[str, object]:
    ...
    return {
        "type": (
            "capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_downstream_task"
        ),
        ...
        "non_claims": [
            ...
            "does_not_mutate_downstream_result_effect_task_result_effect_task_result_records",
        ],
    }
...
def _task_evidence(effect: Effect) -> dict[str, object]:
    ...
    return {
        ...
        "source_delegation_id": result.get("source_delegation_id", "none"),
        "source_downstream_task_id": result.get("source_downstream_task_id", "none"),
        "upstream_source_delegation_id": result.get(
            "upstream_source_delegation_id",
            "none",
        ),
        ...
        "task_origin": (
            "applied_downstream_result_effect_task_result_effect_task_result_decision_effect"
        ),
    }
