...
def list_coder_prep_packets(
    ...
    *,
    ...
    for path in _coder_prep_packet_paths(root, delegation_ids=delegation_ids):
        try:
...
def _coder_prep_packet_paths(
    root: Path,
    *,
    delegation_ids: Iterable[str] | None,
) -> list[Path]:
    if delegation_ids is None:
        return sorted(root.glob(".clanker/delegations/*/runs/*/coder_prep/coder_prep.json"))
    ...
    for delegation_id in requested:
        ...
        try:
            ...
            continue
        paths.extend(delegation_root.glob("runs/*/coder_prep/coder_prep.json"))
    return sorted(paths)
