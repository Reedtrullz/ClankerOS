...
TASK_TYPE = "capability_activation_followup_result_task_result_effect_task"
GOAL_DESCRIPTION = (
    "Prepare downstream proof work from applied downstream result decision effects."
)


def write_capability_activation_followup_result_task_result_effect_tasks(
    root: Path,
) -> tuple[
    Path,
    CapabilityActivationFollowupResultTaskResultEffectTaskBatch,
    ...
    applied_effects = _applied_downstream_result_decision_effects(storage)
    ...
    batch = (
        storage.record_capability_activation_followup_result_task_result_effect_task_batch(
            status=status,
    ...
    report_path.write_text(
        render_capability_activation_followup_result_task_result_effect_tasks_report(
            batch,
            created_tasks,
            existing_tasks,
            applied_effects,
...
def render_capability_activation_followup_result_task_result_effect_task_batch_line(
    batch: CapabilityActivationFollowupResultTaskResultEffectTaskBatch,
) -> str:
    return (
...
def render_capability_activation_followup_result_task_result_effect_tasks_report(
    batch: CapabilityActivationFollowupResultTaskResultEffectTaskBatch,
    created_tasks: list[Task],
    ...
    lines.extend(["", "## Applied Downstream Result Decision Effects", ""])
    if applied_effects:
        lines.extend(
            render_capability_activation_followup_result_task_result_effect_proposal_line(
                effect
...
def _applied_downstream_result_decision_effects(storage: Storage) -> list[Effect]:
    return [
        effect
...
def _task_description(effect: Effect) -> str:
    return (
        ...
        f"from applied downstream result decision effect {effect.id}; keep "
        "activation blocked until fresh proof and explicit operator approval exist."
    )
...
def _verification_plan(effect: Effect) -> dict[str, object]:
    ...
    return {
        "type": "capability_activation_followup_result_task_result_effect_downstream_task",
        ...
        "upstream_followup_effect_id": result.get("source_effect_id", "none"),
        ...
        "non_claims": [
            ...
            "does_not_mutate_downstream_result_task_records",
        ],
    }
...
def _task_evidence(effect: Effect) -> dict[str, object]:
    ...
    return {
        ...
        "task_origin": "applied_downstream_result_decision_effect",
    }
