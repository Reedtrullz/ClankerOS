# Tutorial: Route Downstream Result Effect Task Result Effect Tasks
...
rows. The command routes those downstream proof tasks to read-only evaluator
delegation packets so the next evidence plan can be reviewed without enabling
the capability.
...
```text
task_type=capability_activation_followup_result_task_result_effect_task_result_effect_task
status=pending
...
```text
capability_activation_followup_result_task_result_effect_task_result_effect_task_delegations: capability_activation_followup_result_task_result_effect_task_result_effect_task_delegations_recorded
downstream_tasks: 1
...
and create zero new routing decisions or delegations when every pending
downstream task already has a packet.
...
- `.clanker/delegations/<task-id>-plan-next-downstream-result-effect-task-result-effect-proof-evidence-for-<capability>.json`
