...
- `/goals` - daily goal cockpit. It separates active, paused, and completed
  ...
  or title without writing state or running commands, and remembers query,
...
- `/goals/<goal_id>` - goal-centered workbench that is content-first: the Goal
  ...
  the manual publish handoff is ready. Goal-origin review, commit-request,
  local-commit, publication-request, and publication-handoff forms submit a
  safe `return_to=/goals/<goal_id>` value, so confirmed action results and
  saved workspace resume surfaces return to the Goal's next decision boundary
  after local artifacts are written. Run-origin actions still fall back to the
  run detail page.
  A
...
- Every app page includes a shared operator shell with a global
  ...
  keeps the day-to-day routes visible (`Dashboard`, `Today`, `Guide`,
  `Resume`, `Goals`, `Search`, `Workspace`) and moves advanced routes under
  `More`, which opens automatically when the current route is in that
  secondary group. It includes a visible `Keys` control and the `?` shortcut
  for opening a browser-local keyboard-help dialog, so shortcut discovery is
  ...
  so recent route hops can be reopened from the keyboard launcher. A
  browser-local Last Artifact result also reads
  `localStorage:clankeros-last-artifact` after load, letting the keyboard
...
`Action Preflight` for the local action, return route, expected write,
submitted project/Goal context, field count, and zero-effect boundary, followed
by the existing read-only `Action Confirmation Review` with visible Confirm,
Requires, Writes, Scope, and Boundary cards, a stable
`#action-confirm-local-action` target, and collapsed review evidence. The
existing `Action Confirmation Command Bar` remains as collapsed evidence
