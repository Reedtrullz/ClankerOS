# Tutorial: Route Downstream Result Effect Tasks To Delegation Packets

Use this after
`capability-activation-followup-result-task-result-effect-tasks` has created
pending `capability_activation_followup_result_task_result_effect_task` rows.
The command routes those downstream proof tasks to read-only evaluator
...
```text
capability_activation_followup_result_task_result_effect_task_delegations: capability_activation_followup_result_task_result_effect_task_delegations_recorded
downstream_tasks: 1
...
- `.clanker/delegations/<task-id>-plan-next-downstream-proof-evidence-for-<capability>.json`
