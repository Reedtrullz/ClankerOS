# Tutorial: Create Downstream Result Effect Task Result Effect Proposals
...
downstream result effect task result records. The proposal command creates
generic local `effects` rows so the next local application step can be
reviewed and tracked without enabling a capability or satisfying proof.

## Prerequisites

Create and review downstream result effect task results first:
...
```text
capability_activation_followup_result_task_result_effect_task_result_effect_proposals: capability_activation_followup_result_task_result_effect_task_result_effect_proposals_recorded
...
- source decision and downstream result effect task result ids
- the proposed generic `effects` rows and idempotency keys
- capability, task, result, delegation, and evidence references
