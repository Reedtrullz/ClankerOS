...
- Local operator app: `app` / `local-app` / `serve` starts a standard-library
  ...
  posture. Saved resume surfaces still win; otherwise populated sessions route
  the Resume card to the current Goal action, while first-run sessions keep
  ...
  evidence records each step's action id and visible link label. Completed
...
opens with a visible `Search Operator Workbench` before shared route/focus
  ...
  `Skills Usage Map` follows with Now, Available, Generated, Usage, Last Used,
  Projects, and Safety cards; the Now card's primary action names the selected skill
  ...
  next local action, and the status-artifact-only safety boundary, plus a
  read-only `Health CI Boundary` for current checkout proof, Fast Smoke,
  Full pytest suite, latest local CI proof, and operator-supplied/no-polling
  safety before the collapsed command/diagnostic evidence; a read-only
