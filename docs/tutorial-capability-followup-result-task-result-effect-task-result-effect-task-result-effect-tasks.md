# Tutorial: Materialize Downstream Result Effect Task Result Effect Task Result Effect Tasks
...
decision effects as local ledger records. The task command turns those applied
effects into pending high-risk proof tasks so the next evidence work is queued
in the task graph instead of living only in a report.
...
```text
capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_tasks: capability_activation_followup_result_task_result_effect_task_result_effect_task_result_effect_tasks_recorded
...
- This does not mutate downstream result effect task result effect task result records.
