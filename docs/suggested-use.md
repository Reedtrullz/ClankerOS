...
for the exact expected proposal report, idempotency check, and safety counters.
...
for the application report and idempotency check.

After those local application records exist, create the next pending proof
tasks:
...
After the latest proposed effect row exists, apply it as a local application
record:
