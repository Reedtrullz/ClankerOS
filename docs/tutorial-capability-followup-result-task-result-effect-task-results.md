# Tutorial: Ingest Downstream Result Effect Task Results
...
Create a downstream result effect proof task and delegation packet first:
...
Find the downstream result effect task delegation id in:
...
```text
capability_activation_followup_result_task_result_effect_task_results: capability_activation_followup_result_task_result_effect_task_results_recorded
...
- `docs/capability-activation-followup-result-task-result-effect-task-results/<delegation>-<capability>.json`
- `docs/dashboard.md`, section
  `## Capability Activation Follow-Up Result Task Effect Task Results`

The JSON artifact preserves:

- source application, downstream decision, downstream result, upstream
  follow-up result, source effect, downstream task, contract, project, and
  capability ids
- the structured delegation output
- the downstream result effect task evidence and verification plan
